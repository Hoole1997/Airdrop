...
class ChainOperaTimingWorker(context: Context, workerParams: WorkerParameters) : Worker(context, workerParams) {
    override fun doWork(): Result {
        val projectId = inputData.getInt("PROJECT_ID", 0)
        val model = BaseService.Companion.modelMap[projectId] as ChainOperaModel?
