...
class ChainOperaAccountModule(val context: Context, val recyclerView: RecyclerView) : IProjectAccountModule<ChainOperaUser, ItemTakerProtocolWalletBinding>() {
    ...
    override fun initDiffCallback(): DiffUtil.ItemCallback<ChainOperaUser> {
        return object : DiffUtil.ItemCallback<ChainOperaUser>() {
            override fun areItemsTheSame(
                oldItem: ChainOperaUser,
                newItem: ChainOperaUser
            ): Boolean {
    ...
    override fun initItemView(data: ChainOperaUser?, position: Int, itemDb: ItemTakerProtocolWalletBinding) {
        ...
        data?.let {
            ...
            itemDb.tvPoints.text = "Pts:${BigDecimal(it.totalPoints).toInt()}"
