...
class TakerSowingConverter : JSONConvert(code = "code", message = "message", success = "200") {
    ...
    override fun <R> String.parseBody(succeed: Type): R? {
        ...
        val resultJson = jsonObject.opt("result")

        // 根据result的类型进行解析
        return when (resultJson) {
            is Boolean -> {
                // 如果目标类型是Boolean，直接转换
                if (succeed.typeName == "java.lang.Boolean" || succeed.typeName == "kotlin.Boolean") {
                    resultJson as R
                } else {
                    // 如果目标类型不是Boolean，但返回值是Boolean，可能需要特殊处理
                    // 比如返回null或抛出异常
                    null
                }
            }
                ...
                // 其他类型，尝试解析
                GsonUtils.fromJson(jsonObject.optString("result"), succeed)
            }
        }
    }
